...
def main(waymo_root, output_dir, workers=1, front_only=False, dummy_depth=False):
    extract_frames(waymo_root, output_dir, workers=workers, front_only=front_only)
...
def extract_frames(db_root, output_dir, workers=8, front_only=False):
    sequences = _list_sequences(db_root)
    output_tmp = osp.join(output_dir, "tmp")
    print(">> outputing result to", output_tmp)
    args = [(db_root, output_tmp, seq, front_only) for seq in sequences]
    parallel_map(process_one_seq, args, star_args=True, workers=workers)


def process_one_seq(db_root, output_dir, seq, front_only=False):
    ...
    try:
        with tf.device("/CPU:0"):
            calib = extract_frames_one_seq(
                osp.join(db_root, seq), out_dir, front_only=front_only
            )
    except RuntimeError:
...
def extract_frames_one_seq(filename, out_dir, front_only=False):
    """
    ...
    for f_idx, data in enumerate(tqdm(dataset, leave=False)):
        ...
        if not "show full point cloud":
            show_raw_pointcloud(
                [v for v in [points_all]],  # cheap preview
                [tf.image.decode_jpeg(img.image).numpy() for img in frame.images],
            )

        # free large per-frame variables promptly
        del points, cp_points, points_all, cp_points_all, cp_points_all_tensor
